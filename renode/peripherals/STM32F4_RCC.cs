...
namespace Antmicro.Renode.Peripherals.Miscellaneous
{
    ...
    public sealed class STM32F4_RCC_CUSTOM : IDoubleWordPeripheral, IKnownSize, IProvidesRegisterCollection<DoubleWordRegisterCollection>
    {
        public STM32F4_RCC_CUSTOM(IMachine machine, STM32F4_RTC rtcPeripheral)
        {
            ...
            var registersMap = new Dictionary<long, DoubleWordRegister>
            {
                ...
                {(long)Registers.ClockControlAndStatus, new DoubleWordRegister(this, 0x0E000000)
                    ...
                    .WithFlag(24, name: "RMVF",
                        writeCallback: (_, value) =>
                        {
                            if(value)
                            {
                                this.NoisyLog("Clear reset flags");
                                rstReason = ResetReason.None;
                            }
                        })
                    .WithFlag(25, FieldMode.Read, valueProviderCallback: _ => rstReason == ResetReason.PowerOnReset, name: "BORRSTF")
                    .WithFlag(26, FieldMode.Read, valueProviderCallback: _ => rstReason == ResetReason.PowerOnReset, name: "PINRSTF")
                    .WithFlag(27, FieldMode.Read, valueProviderCallback: _ => rstReason == ResetReason.PowerOnReset, name: "PORRSTF")
                    .WithFlag(28, FieldMode.Read, valueProviderCallback: _ => rstReason == ResetReason.SoftwareReset, name: "SFTRSTF")
                    .WithFlag(29, FieldMode.Read, valueProviderCallback: _ => rstReason == ResetReason.IndependentWatchdogReset, name: "IWDGRSTF")
            ...
            rstReason = ResetReason.PowerOnReset;
        }
        ...
        public ResetReason resetReason
        {
            ...
            set
            {
                this.NoisyLog("Set reset reason {0}", value);
                rstReason = value;
            }
        }

        public void SetResetReasonSoftwareReset()
        {
            rstReason = ResetReason.SoftwareReset;
            this.NoisyLog("Set reset reason {0}", rstReason);
        }

        private ResetReason rstReason;
    }
}
