...
#[cfg(not(test))]
pub mod outer {
    //#[cfg_attr(not(test), rtic::app(device = stm32f4xx_hal::pac, dispatchers = [EXTI0, EXTI1, EXTI2]))]
    ...
    mod app {
        ...
        fn init(ctx: init::Context) -> (Shared, Local, init::Monotonics) {
            ...
            let neighbor_cache =
                NeighborCache::new(&mut ctx.local.net_storage.neighbor_storage[..]);
            let routes = Routes::new(&mut ctx.local.net_storage.routes_storage[..]);
            let mut eth_iface =
                InterfaceBuilder::new(eth_dma, &mut ctx.local.net_storage.sockets[..])
